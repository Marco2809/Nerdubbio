    ...
    <section className="mb-8">
      <div className="mb-3 flex items-start justify-between gap-2">
        <div>
          <div className="flex items-center gap-2">
            ...
            <h2 className="text-sm font-bold uppercase tracking-wider">Calendario</h2>
          </div>
          <p className="mt-0.5 text-[11px] text-muted-foreground">
            Prossime uscite delle tue serie · 4 settimane
          </p>
        </div>
      ...
      {!loading && totalEvents === 0 && (
        <div className="glass rounded-2xl p-4 text-sm text-muted-foreground">
          {hasLibraryShows
            ? "Nessuna uscita programmata nelle prossime settimane per le serie che segui."
            : "Aggiungi serie in corso o da vedere per popolare il calendario."}
          {showMovies ? " Nessun film in uscita al cinema in questo periodo." : null}
        </div>
      )}
